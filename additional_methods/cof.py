...
class COF(BaseDetector):
    ...
    def _cof_kNN(self, X):
        ...
        This function uses a kNN based query to optimize both memory and speed at the possible cost of exactness.
        :return: numpy array containing COF scores for observations.
                 The greater the COF, the greater the outlierness.
        """
        #fit NN:
        knn = NearestNeighbors(n_neighbors=self.n_neighbors_+1)
        knn.fit(X)
        ...
        for i in range(X.shape[0]):
            ...
            sbn_path = knn.kneighbors(X[i,:].reshape(1,-1), return_distance=False)[0]
            sbn_path_index[i,:] = sbn_path[1: self.n_neighbors_ + 1]
            cost_desc = np.zeros((self.n_neighbors_))
            for j in range(self.n_neighbors_):
                cost_desc[j] = np.min(minkowski_distance(X[sbn_path[j + 1]],X[sbn_path][:j +1],p=2))
        ...
        for _g in range(X.shape[0]):
            cof_[_g] = (ac_dist[_g] * self.n_neighbors_) / (np.sum(ac_dist[sbn_path_index[_g]]) + 1e-10)
    ...
    def _cof_fast(self, X):
        ...
        for _g in range(X.shape[0]):
            cof_.append((ac_dist[_g] * self.n_neighbors_) / (
                        np.sum(itemgetter(*sbn_path_index[_g])(ac_dist))+ 1e-10)  )
        return np.nan_to_num(cof_)
